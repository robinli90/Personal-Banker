...
namespace System.Drawing.PieChart {
    ...
    public class PieChartControl : System.Windows.Forms.Panel {
        ...
        /// <summary>
        ///   Sets return string in each pie slice.
        /// </summary>
        public string[] Return_Value {
            set { m_return_value = value; }
            get { return m_return_value; }
        }
        ...
        protected override void OnClick(EventArgs e)
        {
            ...
            int index = m_pieChart.FindPieSliceUnderPoint(new PointF(x.X, x.Y));
            if (index != m_highlightedIndex)
            {
                if (isFirstClick)
                {
                    ...
                    click_text = m_texts[m_highlightedIndex];
                }
                else
                {
                    if (milliseconds < 500 && click_text == m_texts[m_highlightedIndex] && Return_Value[m_highlightedIndex].Length > 0)
                    {
                        try
                        {
                            isDoubleClick = true;
                            Financial_Journal.Category_Summary FT = new Financial_Journal.Category_Summary(m_texts[m_highlightedIndex].Split(new string[] { Environment.NewLine }, StringSplitOptions.None)[0], new Point(x.X - 50, x.Y - 50), Return_Value[m_highlightedIndex]);
                            FT.Show();
                            base.OnClick(e);
                        }
                        catch
        ...
        protected override void OnMouseMove(System.Windows.Forms.MouseEventArgs e) {
            ...
            if (LastMousePos == e.Location)
                return;
            else if (ShowToolTip && m_pieChart != null && m_values != null && m_values.Length > 0) {
        ...
        protected override void Dispose(bool disposing) {
            if (!m_disposed) {
                try {
                    if (disposing) {
                        if (m_pieChart != null) m_pieChart.Dispose();
                        if (m_toolTip != null) m_toolTip.Dispose();
                    }
                    m_disposed = true;
                }
                finally {
        ...
        private string[]        m_return_value = null;
